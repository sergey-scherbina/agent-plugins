...
| [`ci-staged`](./ci-staged/commands/ci-staged.md) | N agents landing to one branch are collapsing a shared build box: a pre-merge gate scoped to a lane's own changed modules then their dependents (never the whole family), and one serial post-merge runner that gates the whole build once per batch and is the only pusher, bisecting and reverting the culprit on red. Part A generic, Part B a worked sbt instantiation. | any |
