...
| [`isolate`](./isolate/commands/isolate.md) | **Finding the cause** when a failure lives inside a stack of interacting components, or looks like noise: read the actual log before guessing, bisect into isolated halves, reduce an input safely (by declaration, with the predicate's identifiers pinned, rebuilt well-formed before you trust it), settle our-bug-vs-model with a control. The companion to `bugs`, which *tracks* what this one *finds*. | any |
...
| [`ci-staged`](./ci-staged/commands/ci-staged.md) | N agents landing (or benchmarking) at once are collapsing a shared build box: run only the specific test/module/benchmark your change needs — never the whole family "just in case" — and reserve the full sweep for one serial, locked, once-per-batch post-merge runner that is also the only pusher, bisecting and reverting the culprit on red. Part A generic, Part B a worked sbt instantiation. | any |
